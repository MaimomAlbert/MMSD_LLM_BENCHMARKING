...
def convert_to_conversation(sample):
    prompt = f"Classify the text <{sample['text']}> and the image into one of <Sarcastic, Non-sarcastic>."
    conversation = [
        { "role": "user",
          "content" : [
...
trainer = SFTTrainer(
    ...
    args = SFTConfig(
        ...
        run_name="qwen2-vl-7b-mmsd2-01", # For Weights and Biases
...
model.save_pretrained("qwen2_vl_7b_mmsd2_outputs")
model.save_pretrained_merged("qwen2_vl_7b_mmsd2_outputs_merged", tokenizer, save_method = "merged_16bit")
model.push_to_hub("sinngam-khaidem/qwen2-vl-7b-mmsd2", token = hf_token) # Online saving
tokenizer.push_to_hub("sinngam-khaidem/qwen2-vl-7b-mmsd2", token = hf_token) # Online saving
model.push_to_hub_merged("sinngam-khaidem/qwen2-vl-7b-mmsd2-merged", tokenizer, save_method = "merged_16bit",token = hf_token)