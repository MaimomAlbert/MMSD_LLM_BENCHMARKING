...
class MyDataset(Dataset):
    def __init__(self, mode, text_name, limit=None):
        ...
        for id in self.data.keys():
            # self.data[id]["image_path"] = os.path.join(WORKING_PATH,"dataset_image",str(id)+".png")
            self.data[id]["image_path"] = os.path.join("/home/gpuuser3/sinngam_albert/datasets/mv_clip_eval/images",str(id)+".png")
    def load_data(self, mode, limit):
        ...
        if mode in ["train"]:
            ...
            for data in datas:
                ...
                # if os.path.isfile(os.path.join(WORKING_PATH,"dataset_image",str(image)+".png")):
                #     data_set[image]={"text":sentence, 'label': label}
                #     cnt += 1
                if os.path.isfile(os.path.join("/home/gpuuser3/sinngam_albert/datasets/mv_clip_eval/images",str(image)+".png")):
                    data_set[image]={"text":sentence, 'label': label}
                    cnt += 1
